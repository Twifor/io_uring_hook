...
class CSVParserBase {
   public:
    ...
    virtual int LoadValue(size_t row_id, int field_id, const char* buf,
                          int size) = 0;
};

class EmptyCSVParser : public CSVParserBase {
   public:
    virtual int BeginOfRow(size_t row_id) override { return 0; }
    virtual int EndOfRow(size_t row_id) override { return 0; }
    virtual int LoadValue(size_t row_id, int field_id, const char* buf,
                          int size) override {
        return 0;
    }
...
class EchoCSVParser : public CSVParserBase {
   public:
    ...
    virtual int LoadValue(size_t row_id, int field_id, const char* buf,
                          int size) override {
        // std::cout << "\t" << row_id << " " << field_id << " " <<
        // std::string(buf, size) << std::endl; if (field_id == 2) {
        //     sum += atoi(buf);
        // }
