...
def run(hook, sqpoll, bthread_num, batch_log, disk_name, pthread):
    start_time = time.time()
    hook_str = ""
    sqpoll_str = ""
    ...
    command = f"{cpu} bash -c '{sqpoll_str} {hook_str} ./test {bthread_num} {batch_log} {pthread}'"
    print(command)
    ...
    return execution_time, avg_read_speed, avg_cpu_usage
...
for x in TASKS:
    tm, speed, cpu = run(True, False, x, Batch, Pan, 0)
    HOOK.append(tm)
    HOOK_CPU.append(cpu)
    HOOK_SPEED.append(speed)

    tm, speed, cpu = run(True, True, x, Batch, Pan, 0)
    HOOK_SQPOLL.append(tm)
    HOOK_SQPOLL_CPU.append(cpu)
    HOOK_SQPOLL_SPEED.append(speed)

    tm, speed, cpu = run(False, False, x, Batch, Pan, 0)
    VANILLA.append(tm)
    VANILLA_CPU.append(cpu)
    VANILLA_SPEED.append(speed)

    tm, speed, cpu = run(False, False, x, Batch, Pan, 1)
